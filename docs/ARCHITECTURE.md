...
1. The LLM extracts semantic facts and evidence spans; the numeric/date/spec diff is deterministic wherever possible.
...
5. Local development uses SQLite for zero-friction reproducibility. The public production runtime uses Cloud Firestore through a dedicated least-privilege service identity and the same typed transactional case model.
...
- Cloud Firestore stores case documents and audit history using its default-database free quota.
- The authenticated task endpoint is implemented for cron/scheduler integration.
- Model credentials can be connected via environment variables or platform secrets.
